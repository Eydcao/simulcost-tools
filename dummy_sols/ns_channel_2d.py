...
def find_optimal_mesh_x(profile, initial_mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold, length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance, max_iter):
    ...
    for i in range(max_iter):
        ...
        cost_i, num_steps = run_sim_ns_channel_2d(profile, current_mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold)
        ...
        if len(param_history) > 1:
            ...
            is_converged, _, _, _, _, _ = compare_res_ns_channel_2d(
                profile, prev_mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                profile, current_mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
...
def grid_search_mesh_x(profile, mesh_x_values, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold, length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance):
    ...
    for mesh_x in mesh_x_values:
        ...
        cost_i, _ = run_sim_ns_channel_2d(profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold)
        cost_history.append(cost_i)
        param_history.append({
            "mesh_x": mesh_x,
            "mesh_y": mesh_y,
        ...
        if len(param_history) > 1:
            ...
            is_converged, _, _, _, _, _ = compare_res_ns_channel_2d(
                profile, prev_mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance
            )
            if is_converged:
                print(f"Convergence achieved between mesh_x {prev_mesh_x} and {mesh_x}")
                best_mesh_x = prev_mesh_x
                converged = True
                break
            else:
...
def find_optimal_mesh_y(profile, mesh_x, initial_mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold, length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance, max_iter):
    ...
    for i in range(max_iter):
        ...
        # Run simulation and track cost
        cost_i, num_steps = run_sim_ns_channel_2d(profile, mesh_x, current_mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold)
        cost_history.append(cost_i)
        param_history.append({
            "mesh_x": mesh_x,
        ...
        if len(param_history) > 1:
            ...
            # Compare with previous results
            is_converged, _, _, _, _, _ = compare_res_ns_channel_2d(
                profile, mesh_x, prev_mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                profile, mesh_x, current_mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance
            )
            if is_converged:
...
def grid_search_omega_u(profile, mesh_x, mesh_y, omega_u_values, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold, length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance):
    ...
    for omega_u in omega_u_values:
        ...
        cost_i, num_steps = run_sim_ns_channel_2d(profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold)
        cost_history.append(cost_i)
        param_history.append({
            "mesh_x": mesh_x,
            "mesh_y": mesh_y,
        ...
        if len(param_history) > 1:
            ...
            is_converged, _, _, _, _, _ = compare_res_ns_channel_2d(
                profile, mesh_x, mesh_y, prev_omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance
            )
            if is_converged:
                print(f"Convergence achieved between omega_u {prev_omega_u} and {omega_u}")
                best_omega_u = prev_omega_u
                converged = True
                break
            else:
...
def grid_search_omega_v(profile, mesh_x, mesh_y, omega_u, omega_v_values, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold, length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance):
    ...
    for omega_v in omega_v_values:
        ...
        if len(param_history) > 1:
            ...
            is_converged, _, _, _, _, _ = compare_res_ns_channel_2d(
                profile, mesh_x, mesh_y, omega_u, prev_omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance
            )
            if is_converged:
                print(f"Convergence achieved between omega_v {prev_omega_v} and {omega_v}")
                best_omega_v = prev_omega_v
                converged = True
                break
            else:
...
def grid_search_omega_p(profile, mesh_x, mesh_y, omega_u, omega_v, omega_p_values, diff_u_threshold, diff_v_threshold, res_iter_v_threshold, length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance):
    ...
    for omega_p in omega_p_values:
        ...
        if len(param_history) > 1:
            ...
            is_converged, _, _, _, _, _ = compare_res_ns_channel_2d(
                profile, mesh_x, mesh_y, omega_u, omega_v, prev_omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance
            )
            if is_converged:
                print(f"Convergence achieved between omega_p {prev_omega_p} and {omega_p}")
                best_omega_p = prev_omega_p
                converged = True
                break
            else:
...
def find_optimal_diff_u_threshold(profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, initial_diff_u_threshold, diff_v_threshold, res_iter_v_threshold, length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance, max_iter):
    ...
    for i in range(max_iter):
        ...
        cost_i, num_steps = run_sim_ns_channel_2d(profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, current_diff_u_threshold, diff_v_threshold, res_iter_v_threshold)
        cost_history.append(cost_i)
        param_history.append({
            "mesh_x": mesh_x,
            "mesh_y": mesh_y,
        ...
        if len(param_history) > 1:
            ...
            is_converged, _, _, _, _, _ = compare_res_ns_channel_2d(
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, prev_diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, current_diff_u_threshold, diff_v_threshold, res_iter_v_threshold,
                length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance
            )
            if is_converged:
...
def find_optimal_diff_v_threshold(profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, initial_diff_v_threshold, res_iter_v_threshold, length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance, max_iter):
    ...
    for i in range(max_iter):
        ...
        cost_i, num_steps = run_sim_ns_channel_2d(profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, current_diff_v_threshold, res_iter_v_threshold)
        cost_history.append(cost_i)
        param_history.append({
            "mesh_x": mesh_x,
            "mesh_y": mesh_y,
        ...
        if len(param_history) > 1:
            ...
            is_converged, _, _, _, _, _ = compare_res_ns_channel_2d(
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, prev_diff_v_threshold, res_iter_v_threshold,
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, current_diff_v_threshold, res_iter_v_threshold,
                length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance
            )
            if is_converged:
...
def find_optimal_res_iter_v_threshold(profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, initial_res_iter_v_threshold, length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance, max_iter):
    ...
    for i in range(max_iter):
        ...
        cost_i, num_steps = run_sim_ns_channel_2d(profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, current_res_iter_v_threshold)
        cost_history.append(cost_i)
        param_history.append({
            "mesh_x": mesh_x,
            "mesh_y": mesh_y,
        ...
        if len(param_history) > 1:
            ...
            is_converged, _, _, _, _, _ = compare_res_ns_channel_2d(
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, prev_res_iter_v_threshold,
                profile, mesh_x, mesh_y, omega_u, omega_v, omega_p, diff_u_threshold, diff_v_threshold, current_res_iter_v_threshold,
                length, breadth, mass_tolerance, u_rmse_tolerance, v_rmse_tolerance, p_rmse_tolerance
            )
            if is_converged:
...
if __name__ == "__main__":
    ...
    # Parameter values
    parser.add_argument("--mesh_x", type=float, default=100, help="Mesh size in x direction")
    parser.add_argument("--mesh_y", type=float, default=20, help="Mesh size in y direction")
