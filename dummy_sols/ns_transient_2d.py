...
def grid_search_resolution(profile, boundary_condition, resolution_values, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization, norm_rmse_tolerance, other_params=None):
    param_history = []
    cost_history = []
    converged = False
    ...
    for i in range(len(resolution_values)):
        ...
        cost_i, _ = run_sim_ns_transient_2d(profile, boundary_condition, resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization, other_params)
        ...
        if len(param_history) > 1:
            ...
            is_converged, _ = compare_res_ns_transient_2d(
                profile, boundary_condition, prev_resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization,
                profile, boundary_condition, resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization,
                norm_rmse_tolerance,
...
def grid_search_cfl(profile, boundary_condition, resolution, reynolds_num, cfl_values, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization, norm_rmse_tolerance, other_params=None):
    param_history = []
    cost_history = []
    converged = False
    best_cfl = None
    ...
    for cfl in cfl_values:
        ...
        if len(param_history) > 1:
            ...
            is_converged, _ = compare_res_ns_transient_2d(
                profile, boundary_condition, resolution, reynolds_num, prev_cfl, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization,
                profile, boundary_condition, resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization,
                norm_rmse_tolerance,
                other_params, other_params
            )
...
def grid_search_relaxation_factor(profile, boundary_condition, resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor_values, residual_threshold, total_runtime, no_dye, cpu, visualization, norm_rmse_tolerance, other_params=None):
    param_history = []
    cost_history = []
    converged = False
    ...
    for relaxation_factor in relaxation_factor_values:
        ...
        if len(param_history) > 1:
            ...
            is_converged, _ = compare_res_ns_transient_2d(
                profile, boundary_condition, resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, prev_relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization,
                profile, boundary_condition, resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization,
                norm_rmse_tolerance,
                other_params, other_params
            )
...
def grid_search_residual_threshold(profile, boundary_condition, resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold_values, total_runtime, no_dye, cpu, visualization, norm_rmse_tolerance, other_params=None):
    param_history = []
    cost_history = []
    converged = False
    ...
    for residual_threshold in residual_threshold_values:
        ...
        if len(param_history) > 1:
            ...
            is_converged, _ = compare_res_ns_transient_2d(
                profile, boundary_condition, resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor, prev_residual_threshold, total_runtime, no_dye, cpu, visualization,
                profile, boundary_condition, resolution, reynolds_num, cfl, advection_scheme, vorticity_confinement, relaxation_factor, residual_threshold, total_runtime, no_dye, cpu, visualization,
                norm_rmse_tolerance,
                other_params, other_params
            )
