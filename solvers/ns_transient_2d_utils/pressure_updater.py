...
class RedBlackSorPressureUpdater(PressureUpdater):
    """Red-Black SOR Method with fixed iterations"""
...
@ti.data_oriented
class ResidualBasedSorPressureUpdater(PressureUpdater):
    """Red-Black SOR Method with residual-based convergence"""

    def __init__(self, boundary_condition, dt, dx, relaxation_factor, residual_threshold, max_iter=1000):
        super().__init__(boundary_condition, dt, dx)
    ...
    def update(self, p, v_current):
        ...
        while iteration < self._max_iter and residual > self._residual_threshold:
            # Store previous pressure for residual calculation
            p_prev = p.current.to_numpy().copy()
            ...
            # Calculate residual
            p_current_np = p.current.to_numpy()
