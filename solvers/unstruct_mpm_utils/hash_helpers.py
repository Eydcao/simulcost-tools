...
def line_box_intersect(bbox, p1, p2):
    ...
    for i in range(3):
        ...
        if (
            (intp0 >= tbbox_min - eps).all() and (intp0 <= tbbox_max + eps).all() and s[0] >= -eps and s[0] <= 1 + eps
        ) or (
            (intp1 >= tbbox_min - eps).all() and (intp1 <= tbbox_max + eps).all() and s[1] >= -eps and s[1] <= 1 + eps
        ):
            return True
...
if __name__ == "__main__":
    ...
    candidate_box = np.array([0.0, 0.0])
    dx = 1.0
    # cell nodes in box
    ...
    assert triangle_box_intersect(candidate_box, dx, cell_pos) == True
    # box in tri
    cell_pos = np.array([[-0.5, -0.5], [1.0, 0.0], [0.0, 1.0]])
