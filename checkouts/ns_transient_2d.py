...
def plot_statistics(statistics, output_dir):
    ...
    if statistics["optimal_resolution_values"]:
        ...
        if most_common_resolution:
            ...
            for bar, count in zip(bars5, counts):
                height = bar.get_height()
                ax5.text(bar.get_x() + bar.get_width() / 2.0, height + 0.01, str(count), ha="center", va="bottom")
    else:
        ax5.text(
            0.5,
            0.5,
            "No optimal resolution\nvalues found",
            ha="center",
            va="center",
    ...
    if statistics["optimal_cfl_values"]:
        ...
        if most_common_cfl:
            ...
            for bar, count in zip(bars6, counts):
                height = bar.get_height()
                ax6.text(bar.get_x() + bar.get_width() / 2.0, height + 0.01, str(count), ha="center", va="bottom")
    else:
        ax6.text(
            0.5, 0.5, "No optimal CFL\nvalues found", ha="center", va="center", transform=ax6.transAxes, fontsize=12
        )
    ...
    if statistics["optimal_relaxation_factor_values"]:
        ...
        if most_common_relaxation:
            ...
            for bar, count in zip(bars7, counts):
                height = bar.get_height()
                ax7.text(bar.get_x() + bar.get_width() / 2.0, height + 0.01, str(count), ha="center", va="bottom")
    else:
        ax7.text(
            0.5,
            0.5,
            "No optimal relaxation\nfactor values found",
            ha="center",
            va="center",
    ...
    if statistics["optimal_residual_threshold_values"]:
        ...
        if most_common_residual:
            ...
            for bar, count in zip(bars8, counts):
                height = bar.get_height()
                ax8.text(bar.get_x() + bar.get_width() / 2.0, height + 0.01, str(count), ha="center", va="bottom")
    else:
        ax8.text(
            0.5,
            0.5,
    ...
    ax9.text(0.5, 0.5, "Reserved for\nfuture analysis", ha="center", va="center", transform=ax9.transAxes, fontsize=12)
    ...
    for bar, count in zip(bars10, convergence_counts):
        height = bar.get_height()
        ax10.text(bar.get_x() + bar.get_width() / 2.0, height + 0.01, str(count), ha="center", va="bottom")
    ...
    for bar, cost in zip(bars11, avg_costs):
        height = bar.get_height()
        ax11.text(
            bar.get_x() + bar.get_width() / 2.0, height * 1.1, f"{cost:.0f}", ha="center", va="bottom", fontsize=8
        )
    ...
    for param_name, values in [
        ("resolution", statistics["optimal_resolution_values"]),
    ...
    # Create table
    table = ax12.table(
        cellText=summary_data, colLabels=["Parameter", "Most Common", "Frequency"], cellLoc="center", loc="center"
    )
    ...
    with open(os.path.join(output_dir, "ns_transient_2d_statistics_summary.txt"), "w") as f:
        ...
        f.write(
            f"Overall convergence rate: {(statistics['total_converged']/max(statistics['total_tasks'], 1)*100):.2f}%\n\n"
        )
...
def process_task(
    ...
    if target_param == "resolution":
        ...
        if "exact_values" in target_config:
            ...
            residual_threshold_values = np.logspace(
                np.log10(float(search_range_min)), np.log10(float(search_range_max)), search_range_slice_num
            )
    ...
    task_record = {
        ...
        "precision_config": {"norm_rmse_tolerance": precision_vals["norm_rmse_tolerance"]},
        "target_config": {
