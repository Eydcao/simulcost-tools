...
from dummy_sols.plate_with_a_hole import find_convergent_nx, find_convergent_ny, find_optimal_nx, find_optimal_ny
...
def save_datasets(successful_tasks, failed_tasks, output_dir):
    ...
    success_dir = os.path.join(output_dir, "plate_with_a_hole", "successful")
    failed_dir = os.path.join(output_dir, "plate_with_a_hole", "failed")
...
def plot_statistics(statistics, output_dir):
    ...
    fig.suptitle("Plate with Hole Dummy Search Statistics", fontsize=16)
    ...
    if statistics["optimal_ny_values"]:
        ny_values, ny_counts = np.unique(list(statistics["optimal_ny_values"]), return_counts=True)
        ax.bar(
            [str(n) for n in ny_values],
            ny_counts,
            alpha=0.7,
            label="ny parameter",
            color=colors[color_idx % len(colors)],
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "plate_with_a_hole_statistics.png"), dpi=300, bbox_inches="tight")
    plt.close()

    # Create detailed statistics file
    stats_file = os.path.join(output_dir, "plate_with_a_hole_statistics_summary.txt")
    with open(stats_file, "w") as f:
        f.write("=== Plate with Hole Dummy Search Statistics Summary ===\n\n")
        ...
        if statistics["optimal_ny_values"]:
            ny_values, ny_counts = np.unique(list(statistics["optimal_ny_values"]), return_counts=True)
            f.write("   ny parameter (iterative):\n")
            for ny, count in zip(ny_values, ny_counts):
                f.write(f"     ny={ny}: {count} times\n")


def main():
    print("=== Plate with Hole Dummy Solution Generation ===")
    print("Loading configuration from plate_with_a_hole.yaml...")

    # Load configuration from YAML
    config_path = os.path.join(os.path.dirname(__file__), "plate_with_a_hole.yaml")
    ...
    for name, info in config["precision_levels"].items():
        ...
        if isinstance(info["tolerance_error"], (int, float)):
            precision_configs[name] = {
                "tolerance_error": info["tolerance_error"],
                "individual_error_tolerance": info["individual_error_tolerance"],
            }
                ...
                for combination in itertools.product(*param_values):
                    ...
                    if target_param == "nx":
                        is_converged, best_param, cost_history, param_history = find_convergent_nx(
                            ...
                            ny=task_params["ny"],
                            tolerance_error=precision_vals["tolerance_error"],
                            individual_error_tolerance=precision_vals["individual_error_tolerance"],
                    ...
                    elif target_param == "ny":
                        is_converged, best_param, cost_history, param_history = find_convergent_ny(
                            profile=profile,
                            nx=task_params["nx"],
                            ny=target_config["initial_value"],
                            tolerance_error=precision_vals["tolerance_error"],
                            individual_error_tolerance=precision_vals["individual_error_tolerance"],
                            multiplication_factor=target_config["multiplication_factor"],
                            max_iteration_num=target_config["max_iteration_num"],
                        ...
                        if best_param is not None:
                            statistics["optimal_ny_values"].append(best_param)
