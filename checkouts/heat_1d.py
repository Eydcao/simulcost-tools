...
from dummy_sols.heat_1d import find_convergent_cfl, find_convergent_n_space
...
def save_datasets(successful_tasks, failed_tasks, output_dir):
    ...
    success_dir = os.path.join(output_dir, "heat_1d", "successful")
    failed_dir = os.path.join(output_dir, "heat_1d", "failed")
...
def plot_statistics(statistics, output_dir):
    ...
    fig.suptitle("Heat 1D Dummy Search Statistics", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "heat_1d_statistics.png"), dpi=300, bbox_inches="tight")
    plt.close()

    # Create detailed statistics file
    stats_file = os.path.join(output_dir, "heat_1d_statistics_summary.txt")
    with open(stats_file, "w") as f:
        f.write("=== Heat 1D Dummy Search Statistics Summary ===\n\n")
        ...
        if statistics["optimal_cfl_values"]:
            cfl_values, cfl_counts = np.unique(list(statistics["optimal_cfl_values"]), return_counts=True)
            f.write("   CFL parameter (0-shot+iterative):\n")
            for cfl, count in zip(cfl_values, cfl_counts):
                f.write(f"     CFL={cfl:.4f}: {count} times\n")

        if statistics["optimal_n_space_values"]:
            n_space_values, n_space_counts = np.unique(list(statistics["optimal_n_space_values"]), return_counts=True)
            f.write("   n_space parameter (0-shot+iterative):\n")
            for n_space, count in zip(n_space_values, n_space_counts):
                f.write(f"     n_space={n_space}: {count} times\n")


def main():
    print("=== Heat 1D Dummy Solution Generation ===")
    print("Loading configuration from heat_1d.yaml...")

    # Load configuration from YAML
    config_path = os.path.join(os.path.dirname(__file__), "heat_1d.yaml")
