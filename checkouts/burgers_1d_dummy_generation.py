...
def plot_statistics(all_results):
    ...
    convergence_rates = [param_convergence_rates[p]['converged'] / max(param_convergence_rates[p]['total'], 1) 
                        for p in params]
    ...
    if len(optimal_param_counts) < 2:
        if not optimal_param_counts:
            ax3.text(0.5, 0.5, 'No converged\noptimal parameters', 
                    ha='center', va='center', transform=ax3.transAxes, fontsize=12)
            ax3.set_title('Optimal Parameter Frequencies')
        
        ax4.text(0.5, 0.5, 'Additional statistics\nnot available', 
    ...
    total_converged = sum(
        sum(1 for task in result['results'] if task.get('converged', False))
        for result in all_results
    )
    ...
    for target_param, (profiles, precision_levels) in target_configs.items():
        ...
        for profile in profiles:
            for precision_level in precision_levels:
                ...
                result = run_single_task(target_param, param_config, profile, 
                                       precision_level, precision_config)
