...
from dummy_sols.compaction import find_convergent_nx, find_convergent_ny, find_optimal_nx, find_optimal_ny
...
def save_datasets(successful_tasks, failed_tasks, output_dir):
    ...
    success_dir = os.path.join(output_dir, "compaction", "successful")
    failed_dir = os.path.join(output_dir, "compaction", "failed")
...
def plot_statistics(statistics, output_dir):
    ...
    fig.suptitle("Compaction Dummy Search Statistics", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "compaction_statistics.png"), dpi=300, bbox_inches="tight")
    plt.close()

    # Create detailed statistics file
    stats_file = os.path.join(output_dir, "compaction_statistics_summary.txt")
    with open(stats_file, "w") as f:
        f.write("=== Compaction Dummy Search Statistics Summary ===\n\n")
...
def main():
    print("=== Compaction Dummy Solution Generation ===")
    print("Loading configuration from compaction.yaml...")

    # Load configuration from YAML
    config_path = os.path.join(os.path.dirname(__file__), "compaction.yaml")
