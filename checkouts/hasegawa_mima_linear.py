...
from dummy_sols.hasegawa_mima_linear import find_convergent_N, find_convergent_dt, find_optimal_cg_atol
...
def save_datasets(successful_tasks, failed_tasks, output_dir):
    ...
    success_dir = os.path.join(output_dir, "hasegawa_mima_linear", "successful")
    failed_dir = os.path.join(output_dir, "hasegawa_mima_linear", "failed")
...
def plot_statistics(statistics, output_dir):
    ...
    fig.suptitle("Hasegawa-Mima Linear Dummy Search Statistics", fontsize=16)
    ...
    if statistics["optimal_N_values"]:
        N_values, N_counts = np.unique(list(statistics["optimal_N_values"]), return_counts=True)
        ax.bar(
    ...
    if statistics["optimal_cg_atol_values"]:
        cg_atol_values, cg_atol_counts = np.unique(list(statistics["optimal_cg_atol_values"]), return_counts=True)
        ax.bar(
            [f"{cg:.1e}" for cg in cg_atol_values],
            cg_atol_counts,
            alpha=0.7,
            label="cg_atol parameter",
            color=colors[color_idx % len(colors)],
        )
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "hasegawa_mima_linear_statistics.png"), dpi=300, bbox_inches="tight")
    plt.close()

    # Create detailed statistics file
    stats_file = os.path.join(output_dir, "hasegawa_mima_linear_statistics_summary.txt")
    with open(stats_file, "w") as f:
        f.write("=== Hasegawa-Mima Linear Dummy Search Statistics Summary ===\n\n")
        ...
        if statistics["optimal_N_values"]:
            N_values, N_counts = np.unique(list(statistics["optimal_N_values"]), return_counts=True)
            f.write("   N parameter (iterative):\n")
            for N, count in zip(N_values, N_counts):
                f.write(f"     N={N}: {count} times\n")

        if statistics["optimal_dt_values"]:
            dt_values, dt_counts = np.unique(list(statistics["optimal_dt_values"]), return_counts=True)
            f.write("   dt parameter (iterative):\n")
            for dt, count in zip(dt_values, dt_counts):
                f.write(f"     dt={dt:.2e}: {count} times\n")

        if statistics["optimal_cg_atol_values"]:
            cg_atol_values, cg_atol_counts = np.unique(list(statistics["optimal_cg_atol_values"]), return_counts=True)
            f.write("   cg_atol parameter (0-shot):\n")
            for cg_atol, count in zip(cg_atol_values, cg_atol_counts):
                f.write(f"     cg_atol={cg_atol:.1e}: {count} times\n")


def main():
    print("=== Hasegawa-Mima Linear Dummy Solution Generation ===")
    print("Loading configuration from hasegawa_mima_linear.yaml...")

    # Load configuration from YAML
    config_path = os.path.join(os.path.dirname(__file__), "hasegawa_mima_linear.yaml")
                ...
                for combination in itertools.product(*param_values):
                    ...
                    if target_param == "N":
                        ...
                        if best_param is not None:
                            statistics["optimal_N_values"].append(best_param)
