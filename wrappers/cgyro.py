...
def runCgyro(profile, n_radial, n_theta, error_tol):
  dir_path = _get_sim_path(
      f"sim_res/cgyro/{profile}_n_radial_{n_radial}_n_theta_{n_theta}_error_tol_{error_tol}"
  )
  ...
  print(f"Running new simulation with parameters: n_radial={n_radial}, n_theta={n_theta}, error_tol={error_tol}")
  runner_path = _find_runner_path()
  if SIM_RES_BASE_DIR:
      dump_dir = os.path.join(SIM_RES_BASE_DIR, f"sim_res/cgyro/{profile}")
      cmd = f"{sys.executable} {runner_path} --config-name={profile} n_radial={n_radial} n_theta={n_theta} error_tol={error_tol} dump_dir={dump_dir}"
  else:
      cmd = f"{sys.executable} {runner_path} --config-name={profile} n_radial={n_radial} n_theta={n_theta} error_tol={error_tol}"
...
def get_res_cgyro(profile, n_radial, n_theta, error_tol):
  dir_path = _get_sim_path(
      f"sim_res/cgyro/{profile}_n_radial_{n_radial}_n_theta_{n_theta}_error_tol_{error_tol}"
  )
  ...
  if not os.path.exists(results_file):
      print(f"No results found for parameters: n_radial={n_radial}, n_theta={n_theta}, error_tol={error_tol}. Triggering simulation.")
      cost, converged = runCgyro(profile, n_radial, n_theta, error_tol)
  else:
      meta_file = os.path.join(dir_path, "meta.json")
      # Load the cost from the meta.json file
      with open(meta_file, "r") as f:
...
def check_convergence_cgyro(profile, n_radial, n_theta, error_tol):
   res, cost, converged = get_res_cgyro(profile, n_radial, n_theta, error_tol)
...
def compare_res_cgyro(profile1, n_radial1, n_theta1, error_tol1, profile2, n_radial2, n_theta2, error_tol2, tolerance):
  res1 = check_convergence_cgyro(profile1, n_radial1, n_theta1, error_tol1)
  res2 = check_convergence_cgyro(profile2, n_radial2, n_theta2, error_tol2)
...
if __name__ == "__main__":
  converged = compare_res_cgyro("p1", 6, 24, 0.001, "p1", 6, 24, 0.001, 1e-4)
