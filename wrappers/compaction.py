...
def _find_runner_path():
    """Automatically find the correct path to compaction.py runner."""
    cwd = os.getcwd()
    ...
    if cwd.endswith('SimulCost-Bench'):
        possible_paths.extend([
            "costsci_tools/runners/compaction.py",
            "runners/compaction.py"
        ])
...
def run_simulation(config_path=None, nx=2, ny=10, verbose=False, **kwargs):
    """
    Run compaction simulation with given parameters.

    Args:
        config_path: Path to YAML config file (optional)
        nx: Number of elements in x direction (typically fixed at 2)
        ny: Number of elements in y direction (main tunable parameter)
...
def compare_convergence(sim_dirs, labels=None, save_path=None):
    ...
    for sim_dir in sim_dirs:
        ...
        if results:
            ...
            ny_values.append(results.get('ny', np.nan))
        else:
            errors.append(np.nan)
            ny_values.append(np.nan)

    # Create comparison plot
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.loglog(ny_values, errors, 'o-', linewidth=2, markersize=8)

    for i, (ny, err, label) in enumerate(zip(ny_values, errors, labels)):
        if not np.isnan(err):
            ax.annotate(label, (ny, err), xytext=(5, 5),
                       textcoords='offset points', fontsize=10)

    ax.set_xlabel('Number of Elements in Y (ny)')
    ax.set_ylabel('Normalized Stress Error')
    ax.set_title('Bar Compaction: Convergence Study')
...
def check_success_criteria(sim_dir, target_error=1e-4, frame=0):
    """
...
def run_parameter_sweep(ny_values, nx=2, config_path=None, **kwargs):
    """
    Run parameter sweep over mesh resolutions.

    Args:
        ny_values: List of ny values to test (main parameter according to CSV)
        nx: Number of elements in x (typically fixed at 2)
    ...
    for ny in ny_values:
        ...
        if sim_result["success"]:
            ...
            sim_dir = f"sim_res/compaction/p1_nx_{nx}_ny_{ny}"
