...
def run_sim_burgers_1d(profile, cfl, k, beta, n_space):
    ...
    dir_path = _get_sim_path(f"sim_res/burgers_1d/{profile}_cfl_{cfl}_k_{k}_beta_{beta}_n_{n_space}/")
    ...
    if SIM_RES_BASE_DIR:
        dump_dir = os.path.join(SIM_RES_BASE_DIR, f"sim_res/burgers_1d/{profile}")
        cmd = f"{sys.executable} {runner_path} --config-name={profile} cfl={cfl} k={k} beta={beta} n_space={n_space} dump_dir={dump_dir}"
    else:
        cmd = f"{sys.executable} {runner_path} --config-name={profile} cfl={cfl} k={k} beta={beta} n_space={n_space}"
