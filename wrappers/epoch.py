...
def runEpoch(profile,nx,dt_mult,nPart,field_order,particle_order):
    dir_path = f"sim_res/epoch/{profile}_nx_{nx}_dtmult_{dt_mult}_part_{nPart}_fieldO_{field_order}_partO_{particle_order}/"
...
def get_res_epoch(profile, nx, dt_mult,nPart, field_order, particle_order):
    ...
    with h5py.File(results_file, "r") as f:
        ...
        results={'Density_Time':f['Density_Time'][:], 'Ey':f['Ey'][:], "Ey_Time":f['Ey_Time'][:], "Density":f['n_elec'][:], "X_Grid":f['x_grid'][:]}
    with open(meta_file, "r") as f:
        meta = json.load(f)
        a0 = meta["a0"]
