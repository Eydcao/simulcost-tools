...
mesh = meshio.read("./projects/FastIPC/input/Sharkey.obj")
...
xPrev = ti.Vector.field(dim, dtype=real)
xTilde = ti.Vector.field(dim, dtype=real)
xn = ti.Vector.field(dim, dtype=real)
v = ti.Vector.field(dim, dtype=real)
m = ti.field(dtype=real)
zero = ti.Vector.field(dim, dtype=real)
restT = ti.Matrix.field(dim, dim, dtype=real)
vertices = ti.field(dtype=ti.i32)
...
data_mat = ti.field(real, shape=(3, 2000000))
...
def compute_energy() -> real:
    ...
    for e in range(n_elements):
        ...
        U, sig, V = svd(F)
        if ti.static(dim == 2):
            total_energy += elasticity_energy(ti.Vector([sig[0, 0], sig[1, 1]]), la, mu) * dt * dt * vol0
        else:
            total_energy += elasticity_energy(ti.Vector([sig[0, 0], sig[1, 1], sig[2, 2]]), la, mu) * dt * dt * vol0
    return total_energy
...
def compute_hessian_and_gradient():
    ...
    for i in range(n_particles):
        for d in ti.static(range(dim)):
            ...
            data_mat[2, c] = m[i]
            data_rhs[i * dim + d] -= m[i] * (x[i][d] - xTilde[i][d])
...
def apply_sol(alpha : real):
    for i in range(n_particles):
        for d in ti.static(range(dim)):
            x[i][d] = xPrev[i][d] + data_sol[i * dim + d] * alpha
