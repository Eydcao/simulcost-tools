...
The slope limiter uses a generalized superbee limiter (updated from Euler implementation):
...
$$r_{j} = \frac{u_{j} - u_{j-1}}{u_{j+1} - u_{j}}$$
...
4. **β-Parameter Optimization (0-shot)**
...
1. **CFL Convergence Search (iterative+0-shot)**
   ...
   - **Non-target parameters**: n_space=2048, with β∈{1.0, 1.5, 2.0} and k∈{-1.0, 0.0, 1.0} combinations
...
| beta | Parameter for generalized superbee limiter strength | beta ≥ 1 |
...
- **Precision Levels**: 3 (high: RMSE≤0.01, medium: RMSE≤0.04, low: RMSE≤0.08)

### Task Distribution

Current configuration generates:

- **CFL** (iterative+0-shot): 5 profiles × 9 non-target combos (k×beta) = 45 tasks
- **n_space** (iterative+0-shot): 5 profiles × 9 non-target combos (k×beta) = 45 tasks  
- **k** (0-shot): 5 profiles × 3 non-target combos (beta) = 15 tasks
- **beta** (0-shot): 5 profiles × 3 non-target combos (k) = 15 tasks
